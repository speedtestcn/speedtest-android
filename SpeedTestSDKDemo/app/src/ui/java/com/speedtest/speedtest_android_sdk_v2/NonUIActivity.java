...
import com.speedmanager.baseapp.log.LogUtil;
...
import static cn.speedtest.speedtest_sdk.SpeedtestFragment.REQ_CODE_CHANGE_NODE;
import static cn.speedtest.speedtest_sdk.testnode.ChangeTestNodeActivity.SELECT_NODE;
...
public class NonUIActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSkipToNodesSelect = findViewById(R.id.btn_skip_to_nodes_select);
        ...
        btnSkipToNodesSelect.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (serverListsBeans == null) {
                    Toast.makeText(NonUIActivity.this, "正在加载节点列表，请稍候", Toast.LENGTH_SHORT).show();
                    return;
                }
                Intent intent = new Intent(NonUIActivity.this, ChangeTestNodeActivity.class);
                //序列化中间出错，get取到对象类型为HashMap，换用json序列号。modify by wr-20200507
                String jsonServerList = new Gson().toJson(serverListsBeans);
                intent.putExtra(ChangeTestNodeActivity.SERVER_LIST, jsonServerList);
                startActivityForResult(intent, REQ_CODE_CHANGE_NODE);
            }
        });
    ...
    private SpeedtestCallback downloadCallback = new SpeedtestCallback() {
        ...
        public void onEnd(double mbps) {
            LogUtil.d("End SpeedTest Download:---------------------次数："+ downCount +"\n"+ mbps);
            Message msg = mHandler.obtainMessage();
            msg.what = 5;
            msg.obj = mbps;
            mHandler.sendMessage(msg);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQ_CODE_CHANGE_NODE) {
            if (resultCode == RESULT_OK) {
                ServerListsBean selectServerNode = data.getParcelableExtra(SELECT_NODE);
                if (selectServerNode != null) {
                    speedtestInterface.setSelectNode(selectServerNode);
                }
