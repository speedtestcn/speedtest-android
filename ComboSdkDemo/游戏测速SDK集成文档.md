本文为你介绍了Android端集成SDK操作，帮助你快速集成SDK并能使用游戏测速基本功能。
...
1. 游戏测速中深度测试为做固定时间的持续Ping测试，其中，Ping失败时不做曲线绘制，数据也不在界面展示。
2. 游戏测速中深度测试，在移动网络时会展示运营商信息，其中，部分品牌手机默认只提供空白通行证，空白通行证时信息无法展示，建议用户手动修改授权。
